...
- 更新链路使用公开 Raw 清单和 SHA-256；下载后仍需用户确认才替换并重启，不嵌入 GitHub Token。
...
1. 读取 `releases/manifest.json` 的 Raw 公共地址。
2. 比较语义版本。
3. 从 GitHub Release 下载版本化 EXE。
4. 核对清单中的 SHA-256，不一致则拒绝安装。
5. 用户确认后，由临时 PowerShell 等待当前程序退出、替换原文件并重新启动。

GitHub API 匿名访问曾在真实测试中返回 403；Raw CDN 在发布后也曾短暂返回旧清单，因此检查地址必须保留防缓存参数。不要恢复成 API 作为唯一来源，也不要去掉防缓存参数。
...
- 更新自检已验证：读取公开清单、下载 1.2.0 Release、SHA-256 一致。
