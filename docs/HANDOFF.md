...
## 2026-07-27 11:22 事件与系统修复

- 11:22:22 连续出现 5 次 AppModel `0x3CFC`，恢复中心 11:22:29 读到包状态为 `Modified, NeedsRemediation`。当时虚拟内存仍有 10.5 GB、提交压力约 67%，且最近三小时无新的资源耗尽事件；因此这次包异常不能归因于当刻内存不足。
- 11:23 用户级注册操作成功，但机器级状态仍未恢复；恢复中心随后调用微软商店官方源，11:26:44 状态从 `0x2` 恢复为 `0x0`，11:26:45 安装成功。恢复中心完整闭环用时约 4 分 16 秒。
- 同一台机器仍有独立的内存崩溃链：00:59 的 `ChatGPT.exe / e0000008` 为确认的 OOM。12:07 已把 D 盘页面文件配置从初始 16 GB、最大 32 GB改为初始 24 GB、最大 32 GB；当前实际分配仍为 16 GB，必须重启后才生效。
- 12:07 执行 DISM `RestoreHealth`，CBS 最终记录 `S_OK`、组件损坏数 0、无需因 DISM 重启；12:13–12:18 执行 SFC，退出码 0、`Repairing 0 components`。广泛 Windows 组件/系统文件损坏已排除。
- 12:18 包状态仍为 `Ok`，修复开始后未新增 Codex AppModel 错误。机器级包状态为何会自行进入 `0x2` 仍没有直接上游事件，保留为厂商包/AppX 状态异常；不得把“商店修复有效”反推为“商店导致故障”。
- 管理员脚本首次启动失败的根因是 Windows PowerShell 5.1 把无 BOM UTF-8 中文脚本按 ANSI 解析。后续管理员脚本内核改为 ASCII，并在 UAC 前用 PowerShell Parser 做语法验收。
