...
这是一个独立于 Codex 运行的 Windows 图形化恢复工具。它处理“应用有问题、请重新安装”、AppX 注册异常、旧脚本黑框闪退，以及只能反复进入微软商店更新才能恢复的问题。它还会区分内存耗尽与 GPU / 会话渲染崩溃：内存不足时提供“释放内存”，旧会话恢复网页状态拉崩 GPU 时指出触发会话并建议安全启动，而不是一律做耗时的商店重装。
...
- **安全模式启动**：安装状态正常，但担心多窗口、旧网页状态或 GPU 渲染再次崩溃时使用。
...
| 1.5.0 | 识别 GPU / 会话渲染崩溃，指出触发会话并避免误走商店重装 |
